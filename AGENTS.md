...
For DSP customer billing, the configured customer midnight surcharge is an hourly fee, not a one-time fee. Use the eight fixed Singapore clock-hour slots from `23:00`–`00:00` through `06:00`–`07:00`; every slot with any verified DSP service overlap is chargeable, including a partial slot. A DSP from `23:30` to `00:30` therefore receives two midnight fees. Fixed-trip MNG, DEP, and TRF retain exactly one configured midnight surcharge when their pickup is inside the `23:00` through `06:59` window. Use the same authoritative saved-pickup-to-JC or latest Admin-correction start/end pair that produced the DSP billed duration; never infer midnight exposure from scheduled end, current time, booking status, invoice text, or another job. Booking `10894` is the locked crossing-midnight reference: corrected `21 Aug 2026 18:35 SGT` to `22 Aug 2026 03:19 SGT` is `524` minutes, `9` billable hours at SGD65/hour = SGD585. Its midnight exposure touches five slots from `23:00`–`00:00` through `03:00`–`04:00`, so the surcharge is `5 × SGD15 = SGD75` and the temporary proposal is `SGD660` before any other separately persisted surcharge. This financial rule is owner-locked because undercounting a touched midnight slot loses owner revenue; future Codex work must not reinterpret it as elapsed whole hours, service-start-aligned hours, one DSP fee, or a prorated amount.
...
Privacy remains mandatory: customers must never see Admin ↔ Driver messages; admin must see Customer ↔ Driver messages; customer reads remain `customer_app` only; driver reads remain `driver_app` only. Ordinary Customer ↔ Driver messages use the existing bounded typed `client_message_id + message_text` payload, exact-booking scope, authenticated Customer principal or active Driver-link boundary, and the existing pre-POB/completion closure. Separate verified PA and Boss credentials share one exact-booking conversation on the same Customer screen; either authorized principal may send, while the Driver sees only the verified Boss name and immutable private audit retains the actual PA/Boss principal and role. One Driver → Customer message creates one existing `customer_app` outbox row that both authorized PA and Boss may read through that shared booking conversation; never duplicate the row per recipient. The existing fixed `Acknowledge driver details` Customer action remains a separate `template_key`-only Customer-to-Admin acknowledgement and must not accept arbitrary text. No Email, WhatsApp, SMS, Telegram, or other external provider send is part of this lane.
