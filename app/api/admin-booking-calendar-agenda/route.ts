import {
  adminBookingCalendarAgendaVersion,
...
function safeFailureResponse() {
  return Response.json(
    {
      error: "Admin booking calendar agenda request failed safely.",
      ok: false,
    },
    { status: 500 },
  );
...
export async function POST(request: Request) {
  try {
    ...
    const result = buildAdminBookingCalendarAgenda(await readJsonBody(request));
