...
import { buildCustomerDriverDetailsEmailReadinessSetup } from "../../../lib/customer-driver-details-email-readiness-setup-foundation";
...
function safeFailureResponse() {
  const send = prepareDisabledAdminEmailSend({});

  return Response.json(
    {
      error: "Customer driver details email disabled send setup request failed safely.",
      external_send: send.external_send,
      ok: false,
      reason: send.reason,
      send,
      sendingEnabled: send.sendingEnabled,
      status: send.status,
