...
import { loadAdminDriverJobStatuses } from "../../../lib/admin-driver-job-status-read";
...
function safeFailureResponse() {
  return Response.json(
    {
      error: "Admin driver job status request failed safely.",
      ok: false,
    },
    { status: 500 },
  );
...
export async function GET(request: Request) {
  try {
    ...
    const result = await loadAdminDriverJobStatuses(new URL(request.url).searchParams, actor);
