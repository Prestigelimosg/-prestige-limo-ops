import {
  adminDriverPayoutRulesRuntimeWriteActionEnvGateName,
  executeAdminDriverPayoutRulesRuntimeWriteAction,
} from "../../../lib/admin-driver-payout-rules-runtime-write-action";
...
function safeFailureResponse() {
  return Response.json(
    {
      ok: false,
      error: "Driver payout rules write request failed safely.",
      env_gate_name: adminDriverPayoutRulesRuntimeWriteActionEnvGateName,
      no_op: true,
      write_enabled: false,
    },
...
export async function POST(request: Request) {
  try {
    ...
    const result = await executeAdminDriverPayoutRulesRuntimeWriteAction(
