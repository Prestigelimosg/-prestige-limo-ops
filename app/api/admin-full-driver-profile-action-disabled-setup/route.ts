import {
  ...
  fallbackAdminFullDriverProfileActionDisabledSetup,
} from "../../../lib/admin-full-driver-profile-action-disabled-setup";
import {
...
function blockedResponse(error: string) {
  const result = fallbackAdminFullDriverProfileActionDisabledSetup();
  ...
  return Response.json(
    {
      ...disabledActionFields(),
      delivery_surface: result.delivery_surface,
      error: "Full driver profile action disabled setup request failed safely.",
      ok: false,
      reason: "setup_only_disabled",
      result,
      status: "blocked",
...
export async function GET(request: Request) {
  try {
    ...
    const result = buildAdminFullDriverProfileActionDisabledSetup(requestInput(request));
