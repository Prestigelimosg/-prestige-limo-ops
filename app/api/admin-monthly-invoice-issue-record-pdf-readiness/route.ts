...
import {
  ...
  parseAdminMonthlyInvoiceIssueRecordPdfReadyPayload,
...
function safeFailureResponse() {
  return Response.json(
    {
      error: "Admin monthly invoice issue record PDF readiness request failed safely.",
      ok: false,
    },
    { status: 500 },
  );
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = parseAdminMonthlyInvoiceIssueRecordPdfReadyPayload(
    ...
    const result = await markAdminMonthlyInvoiceIssueRecordPdfReviewReady(
      parsed.data,
      actor,
    );
