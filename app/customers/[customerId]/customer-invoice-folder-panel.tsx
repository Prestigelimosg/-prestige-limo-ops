...
export function CustomerInvoiceFolderPanel({ customer }: CustomerInvoiceFolderPanelProps) {
  ...
  useEffect(() => {
    ...
    if (searchParams.get("ready_fake_ritz_invoice") !== "1") {
      return;
    }

    const references = String(searchParams.get("selected_booking_references") ?? "")
      .split(",")
      .map((reference) => reference.trim())
      .filter((reference, index, allReferences) =>
        /^FAKE-RITZ-\d{4}$/.test(reference) && allReferences.indexOf(reference) === index,
      );

    if (!/ritz\s+carlton/i.test(customer.companyName) || references.length === 0) {
      return;
    }

    const handoffKey = `prestige.ready-fake-ritz-invoice.${customer.id}.${references.join(".")}`;

    if (window.sessionStorage.getItem(handoffKey) === "done") {
      searchParams.delete("ready_fake_ritz_invoice");
      searchParams.delete("selected_booking_references");
    ...
    async function createReadyInvoice() {
      setStoredInvoiceMessage("Creating pending invoice for Ritz Carlton...");

      try {
        const lineItems = references.map((reference) => ({
          amountLabel: "$420.00",
          description: fakeRitzInvoiceLineDescription(reference),
        }));
        const response = await fetch(adminCustomerInvoicesApiPath, {
          body: JSON.stringify({
            amountCents: references.length * 42000,
            billingMonthLabel: "July 2026",
            customerId: customer.id,
            customerName: customer.companyName,
            documentState: "issued",
            documentType: "invoice",
            dueDateIso: fakeRitzDueDateIso(),
            lineItems,
            reference: references.join(", "),
            route: "Ritz Carlton > Marina Bay Cruise Centre",
            service:
              references.length === 1
                ? fakeRitzInvoiceService(references[0])
                : `${references.length} Ritz Carlton completed jobs`,
            status: "Unpaid",
          }),
          headers: {
        ...
        if (!response.ok || !result?.ok || !result.invoice || cancelled) {
          throw new Error("Pending invoice create failed");
        }

        const invoice = result.invoice as StoredInvoiceRecord;
        const displayInvoice = {
          amount: safeDisplay(invoice.amountLabel, "$0"),
          dueDate: safeDisplay(invoice.dueDateLabel, "Due date to confirm"),
          invoiceNumber: safeDisplay(invoice.invoiceNumber, ""),
          issueDate: safeDisplay(invoice.issueDateLabel, "Date to confirm"),
          lineItems: Array.isArray(invoice.lineItems) ? invoice.lineItems : lineItems,
          route: safeDisplay(invoice.route, "Ritz Carlton > Marina Bay Cruise Centre"),
          service: safeDisplay(invoice.service, "Ritz Carlton completed jobs"),
          status: safeDisplay(invoice.status, "Unpaid"),
        };

        if (displayInvoice.invoiceNumber) {
          setStoredInvoices((currentInvoices) => [
            displayInvoice,
            ...currentInvoices.filter((currentInvoice) => currentInvoice.invoiceNumber !== displayInvoice.invoiceNumber),
          ]);
          setSelectedInvoiceNumber(displayInvoice.invoiceNumber);
        }

        window.sessionStorage.setItem(handoffKey, "done");
        setStoredInvoiceMessage("Loaded pending Ritz Carlton invoice for customer review.");
        searchParams.delete("ready_fake_ritz_invoice");
        searchParams.delete("selected_booking_references");
        window.history.replaceState(
          null,
        ...
        if (!cancelled) {
          setStoredInvoiceMessage("Pending Ritz Carlton invoice could not be created.");
        }
      }
    }

    void createReadyInvoice();
