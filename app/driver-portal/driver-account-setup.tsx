...
export function DriverAccountSetup({pending,onCancel}:{pending:boolean;onCancel:()=>void}) {
  ...
  return <section className="space-y-3 rounded-xl border bg-white p-4" data-driver-account-pending-setup="true">
    ...
    {pending?<><p>Setup saved. You cannot sign in yet.</p><p className="font-semibold">Return to Admin&apos;s message and tap your Job Link. It will open Prestige Driver to activate your account.</p><p>After activation, review the job and tap Save &amp; Acknowledge Job.</p>{!nativeAccountSetupState()?.attempted?<button className="min-h-11 rounded border px-3" disabled={busy} onClick={()=>send("native_account_setup_cancel")}>Cancel setup</button>:null}</>:<form className="space-y-3" onSubmit={e=>{e.preventDefault();send("native_account_setup_save");}}>
