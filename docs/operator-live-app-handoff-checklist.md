...
4. Customer change/cancel request rows show exactly three admin buttons: `Accept + Cal`, `Reject`, and `Dismiss`; there is no separate review/apply/cancel duplicate button lane.
...
6. `Reject` and `Dismiss` mark the booking cancelled, sync the existing Google Calendar event, archive the request, and move the booking to Completed / History.
7. Calendar duplicate rule: make changes in Prestige and use `Update + Cal`, `Accept + Cal`, `Reject`, or `Dismiss`; do not create a separate manual calendar event for the same booking reference.
