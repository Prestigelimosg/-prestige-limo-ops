...
## 17. Stage 4A-159 Billing Readiness And Approval Gates — Docs Only

This section is planning documentation only. It does not approve app behavior, parser behavior, Supabase behavior, migrations, invoice generation, PDF generation, payment links, Stripe, PayNow, bank API behavior, notification sending, customer auth, storage persistence, or runtime code.

The purpose is to make future monthly billing approval points explicit before any real save, linking, invoice numbering, customer billing portal, payment, PDF, notification, or Supabase migration work begins.

### No Approval Yet

The following items remain unapproved and not implemented:

- Supabase billing schema or migration.
- Real monthly billing save.
- Customer billing linking.
- Invoice numbers.
- Invoice or PDF generation.
- Stripe or payment links.
- PayNow API.
- Bank API.
- Email, WhatsApp, or SMS notifications.
- Telegram notifications.
- Customer billing auth or permissions.

### Approval Gates Before Future Real Billing Steps

- Before any Supabase migration plan: the owner must approve a planning-only task that names the intended billing entities, RLS questions, route boundaries, rollback concerns, and test coverage. The plan must not create migration files or run Supabase commands.
- Before any Supabase migration file: the owner must explicitly approve a separate migration-creation stage after reviewing the data model, RLS, service-role boundaries, rollback/recovery plan, and fixture strategy.
- Before any real customer monthly billing save or linking: the selected customer, billing period, line-item mapping, duplicate-save behavior, failure states, audit events, and no-invoice/no-payment boundary must be documented and tested.
- Before any invoice number generation: customer invoice prefix and running-number rules must be agreed, including uniqueness, immutability, sequence gaps, cancellation/void handling, retry behavior, and concurrency safety.
...
- Before any payment link or Stripe behavior: Stripe test-mode scope, secret handling, webhook security, idempotency, payment-status mapping, failure handling, and production-disabled defaults must be approved in a separate payment stage.
- Before any customer billing portal permission or auth behavior: role/permission rules, customer-safe projections, RLS, session handling, cross-customer isolation, and internal-note exclusion must be approved and tested.
- Before any notification behavior: recipient rules, message copy, opt-out or manual-send policy, rate limits, audit logging, failure handling, and no-send test boundaries must be approved separately.

### Readiness Checks Before Implementation

These checks must be complete before future real billing implementation starts:

- Customer invoice prefix and running-number policy agreed.
- Amendment, cancellation, no-show, credit, and void billing rules agreed.
- Paid, partially paid, unpaid, overdue, waived, refunded, and manual bank transfer status rules agreed.
- Audit trail requirements agreed for billing profiles, line items, invoice issuance, payment status changes, manual transfer references, and corrections.
- Role and permission requirements agreed for creating, reviewing, locking, issuing, adjusting, voiding, and recording payment on billing records.
- Test fixtures planned for normal, duplicate, invalid, cross-customer, retry, and rollback cases.
- Rollback/recovery plan documented before any migration or write behavior.
- Protected parser tests still passing, with unrelated parser changes forbidden.
- Browser, mobile, and no-horizontal-overflow checks still passing for `/customers` and protected public/customer/driver routes.

### Locked Business Rules

- Bank wire/transfer remains manual-record only; no bank API, bank scraping, or automatic reconciliation is approved.
- Mock `/customers` UI must not write real billing data.
- Customer payment status must not be changed by mock UI.
- Invoice numbers must not be generated by mock UI.
- Parser reliability remains protected and unrelated parser changes are forbidden.
- Future real billing must be staged with tests before and after each small approved step.
- No package script or `test:safe` membership change is implied by this readiness review.

### Conservative Next-Step Decision List

- If the owner approves only planning, continue docs-only review and keep all runtime behavior unchanged.
- If the owner approves migration planning, create migration plan documentation only; do not create migration files.
- If the owner approves an actual migration later, create it in a separate explicitly approved stage with tests, rollback notes, and no Supabase command unless separately authorized.
- No real billing app code should be added until migration and data model approval is complete.
- No invoice, PDF, Stripe/payment link, PayNow, bank API, notification, or customer billing portal work should be bundled into the first real save/linking step.

This readiness review does not claim real billing works now and does not claim the app is production-ready. It records approval gates and readiness checks for later explicitly approved implementation stages.
