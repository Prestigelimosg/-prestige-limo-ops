...
- [Customer Portal Real Booking Data Path Audit](customer-portal-real-booking-data-path-audit.md) owns the customer saved-bookings read path, customer session issue contract, same-origin cookie handoff, customer-safe field boundary, sample fallback posture, and no-live-write/no-send scope.
