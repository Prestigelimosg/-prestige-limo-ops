...
Approved Job Card Format memory:
- Treat this as reusable interpretation knowledge for the existing structured booking lane, not as another parser, formatter, booking source, or write path.
- Memory examples are interpretation rules only; never copy a historical passenger, contact, company, place, flight, price, or note into the current email result. Every returned fact must come from the complete current email.
- The Email AI result is the richer app booking needed for CRM review, not the short driver-facing WhatsApp Job Card. Preserve every source-supported company/account, Booker, Booker contact/email, passenger/traveller identity/contact, trip, vehicle, pax, bag, and operational-note fact in its separate structured field. Never invent a CRM identity or ID.
- The established Job Card order is vehicle and service, schedule, route, booked passenger quantity, then supported operational notes. Use that meaning to keep each current-email fact in its correct structured field; do not return a formatted Job Card message.
- AVF, VVV, Combi, E-Class, and S-Class are vehicle labels or codes. Preserve the supported current-email vehicle value. A vehicle Passengers count is capacity, never booked pax.
- MNG means an arrival or meet-and-greet: the flight or airport is the pickup side and the stated destination is the drop-off. DEP means an airport departure: the stated street or hotel is the pickup and the flight or airport is the drop-off side. TRF keeps pickup, supported stop or stops, and destination as a point-to-point transfer. DSP keeps hourly, disposal, standby, and itinerary meaning.
- In the current email, Passenger identifies the traveller. Passengers, Pax, or the booking-form typo Passangers under client details identifies booked passenger quantity. Keep passenger identity separate from Booker and company.
- A departure shaped as street pickup plus flight belongs to DEP even when generic text also says Airport transfer or One Way. An arrival shaped as flight plus destination belongs to MNG. Preserve an explicitly stated terminal with its flight or airport, but never invent a terminal.
- When an exact Airport Departure supplies a Singapore street pickup and flight number but no named airport or terminal, complete the structured app booking with dropoff Changi Airport and no terminal. Any explicitly named different airport wins.
- Pickup lobby instructions, language requests, placard instructions, child seats, and genuine supported service remarks belong in operational notes. They never establish a customer company or price.
...
Passenger role, title, employer, or affiliation text inside or beside the labelled Passenger value is passenger context only. For example, a role followed by an organisation inside parentheses after the passenger name is not a Company or Agency label and must never populate companyAccount.
...
For confirmed_booking, amendment, cancellation, or a booking-like enquiry, extract every supported trip into bookingResult using the established service meanings: MNG is an arrival or meet-and-greet pickup from an airport or seaport; DEP is a departure drop-off at an airport or seaport; TRF is a point-to-point transfer that is not an arrival or departure; DSP is hourly, disposal, or standby. An exact labelled Route name Airport Departure always maps to DEP, and Route name Airport Arrival always maps to MNG; generic Service type Airport transfer, Transfer type One Way, or similar wording must never override that explicit route label. Leave unknown fields empty and list uncertainties. For unrelated mail, return an empty bookingResult.

Return-form field checks before output:
- For each explicit Return (new ride) leg, identify evidence of the BOOKED passenger quantity separately from VEHICLE capacity. No booked quantity means pax is an empty string on both legs, with "Confirm booked passenger count; vehicle passenger count is capacity." Never default to capacity, 1, the number of named people, or the number of legs.
- For these return forms, distinguish each phone's role from the person's name and email nearby. An unqualified CLIENT DETAILS Phone number with ambiguous Booker/passenger identity must leave both phone fields empty, with "Confirm client phone contact role: Booker or passenger." Do not duplicate that number across roles. If a phone is explicitly assigned to one role, preserve it only in that role; populate both only when the source explicitly supports both. Do not infer a Booker name from an email or choose a CRM account.
- Synthetic field examples, not facts to copy: "VEHICLE > Passengers count 6; no booked count" gives pax "" plus the booked-count review reason. "VEHICLE > Passengers count 6; CLIENT DETAILS > Passangers 2" gives pax "2". "Client name is traveller; requester identity unclear; Phone number supplied without a role" gives bookerContact "" and passengerContact "" plus the contact-role review reason. These rules do not erase explicit booked counts or role-labelled phones elsewhere in the current email.
- Check both leg objects for these blanks and their specific needsReviewReasons before returning them. Source review is preferable to guessing.`;
