...
import { driverAccountDeviceLockVersion } from "./driver-account-device-lock.ts";
import { issueDriverPortalAccountSession } from "./driver-portal-session.ts";

type Row = Record<string, unknown>;
type Env = Record<string, string | undefined>;
type Client = { rpc: (name: string, args: Row) => Promise<{ data: unknown; error: unknown }> };
...
const messages: Record<string,string> = {
  ...
  account_exists: "An account or setup already exists. If you started setup with another Job Link, reopen that original link. Otherwise use your existing sign-in or ask Admin to review this setup. Do not create another account.",
