...
const approvalEnvName = "PRESTIGE_ADMIN_APP_NOTIFICATION_PRODUCTION_SAVE_LOAD_APPROVED";
const approvalValue = "stage-admin-app-notification-william-approved";
const candidateEnvFileNames = [".env.local", ".env.stage4a388.local"];
const expectedMaskedProductionProjectRef = "kvv...atm";
const fakeBookingReference = "PROD-APP-NOTIFY-VERIFY-20260607-001";
const fakeEventKey = "PROD-APP-NOTIFY-EVENT-20260607-001";
const fakeNotificationType = "system_notice";
const fakeNotificationStatus = "queued";
const fakeNotificationPriority = "normal";
const fakeWorkflowArea = "admin_app_notifications";
const fakeSafeTitle = "Production app notice verification";
const fakeSafeMessage =
  "Fake admin in-app notice verification row; load back, then remove exact row.";
const fakeSafeContext = {
  cleanup_scope: "exact_event_key_and_reference",
  verification_stage: "admin_app_notification_outbox",
};
const liveAttemptMarkerPath = path.join(
  os.tmpdir(),
  "prestige-admin-app-notification-live-write-attempted.marker",
);
...
async function loadAndValidateEnv() {
  ...
  failSafely("admin_app_notification_production_env_preflight_failed", {
...
async function loadHarness() {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "prestige-admin-app-notification-live-"));
  ...
  return {
    ...
    notificationPersistence: require(path.join(tempDir, "lib/admin-app-notification-persistence.js")),
...
function notificationRecordMatchesFake(row) {
  return (
    ...
    row?.delivery_surface === "admin_app" &&
...
async function writeLiveAttemptMarker() {
  try {
    await writeFile(
      ...
      `Admin app notification controlled live write attempted for ${fakeEventKey}\n`,
      {
    ...
    if (error?.code === "EEXIST") {
      failSafely("controlled_admin_app_notification_live_write_already_attempted");
    }
...
async function loadExactCleanupRows(client) {
  const { data, error } = await client
    ...
    .select(
      "id, notification_type, notification_status, priority, delivery_surface, event_key, booking_reference, workflow_area, safe_title, safe_message, safe_context, source_surface, actor_role, actor_label",
    )
    .eq("event_key", fakeEventKey)
    .eq("booking_reference", fakeBookingReference);

  if (error) {
    failSafely("controlled_admin_app_notification_cleanup_preselect_failed_safely", {
...
async function cleanupExactFakeRow() {
  ...
  if (
    rows.length !== 1 ||
    !notificationRecordMatchesFake(rows[0]) ||
    safeRecordContainsUnsafeFields(rows[0])
  ) {
    failSafely("controlled_admin_app_notification_cleanup_exact_match_failed", {
  ...
  const { data, error } = await client
    ...
    .eq("event_key", fakeEventKey)
    .eq("booking_reference", fakeBookingReference)
    .select("event_key, booking_reference, notification_type, notification_status");

  if (error || !Array.isArray(data) || data.length !== 1) {
    failSafely("controlled_admin_app_notification_cleanup_delete_failed_safely", {
  ...
  if (remainingRows.length !== 0) {
    failSafely("controlled_admin_app_notification_cleanup_verify_absent_failed", {
...
async function main() {
  ...
  try {
    const parsed = harness.notificationPersistence.parseAdminAppNotificationCreatePayload(fakePayload());

    if (!parsed.ok) {
      failSafely("safe_admin_app_notification_payload_rejected_before_live_write", {
        verificationEventKey: fakeEventKey,
        verificationReference: fakeBookingReference,
      });
    }
    ...
    const blockedAnonymous = await readResponse(
      await harness.route.GET(
        getRequest(
          `http://localhost/api/admin-app-notifications?booking_reference=${fakeBookingReference}`,
          {},
        ),
      ),
    );
    const blockedCustomerReferer = await readResponse(
      await harness.route.POST(
        requestWithJson("http://localhost/api/admin-app-notifications", fakePayload(), {
          ...adminHeaders(),
    ...
    const unsafePayloadResult = await readResponse(
      await harness.route.POST(
        requestWithJson(
          "http://localhost/api/admin-app-notifications",
          {
            ...fakePayload(),
    ...
    if (
      ...
      failSafely("admin_app_notification_route_safety_gate_failed_before_live_write", {
    ...
    const preExistingLoad = await readResponse(
      await harness.route.GET(
        getRequest(
          `http://localhost/api/admin-app-notifications?booking_reference=${fakeBookingReference}&notification_type=${fakeNotificationType}&limit=10&page=1`,
          adminHeaders(),
        ),
      ),
    );
    ...
    if (
      ...
      failSafely("admin_app_notification_fake_reference_already_exists_or_preload_failed", {
    ...
    const saveResult = await readResponse(
      await harness.route.POST(
        requestWithJson("http://localhost/api/admin-app-notifications", fakePayload(), adminHeaders()),
      ),
    );
    ...
    if (
      ...
      !notificationRecordMatchesFake(saveResult.body.notification) ||
      safeRecordContainsUnsafeFields(saveResult.body.notification)
    ) {
      failSafely("controlled_admin_app_notification_save_failed_safely", {
        productionDbTouched: true,
    ...
    const loadResult = await readResponse(
      await harness.route.GET(
        getRequest(
          `http://localhost/api/admin-app-notifications?booking_reference=${fakeBookingReference}&notification_type=${fakeNotificationType}&notification_status=${fakeNotificationStatus}&limit=10&page=1`,
          adminHeaders(),
        ),
      ),
    );
    const loadedRecord = Array.isArray(loadResult.body?.notifications)
      ? loadResult.body.notifications.find((record) => notificationRecordMatchesFake(record))
      : null;
    ...
    if (
      ...
      failSafely("controlled_admin_app_notification_load_failed_safely", {
        productionDbTouched: true,
    ...
    if (
      ...
      failSafely("controlled_admin_app_notification_post_cleanup_route_load_failed", {
    ...
    if (normalizedEnvValue(finalEnv.PRESTIGE_ADMIN_BOOKING_PERSISTENCE_ENABLED) !== "false") {
      failSafely("local_env_persistence_default_not_off_after_admin_app_notification_verification", {
        productionDbTouched: true,
        verificationEventKey: fakeEventKey,
        verificationReference: fakeBookingReference,
      });
    ...
    emitEvidence({
      cleanupRollback: {
        ...
        cleanupMethod: "Supabase JS exact event_key and booking_reference delete on admin_app_notification_outbox",
      ...
      stage: "admin-app-notification-production-verification",
      targetMatchesPriorProductionEvidence: true,
      touchScope: [
        "one admin-gated POST save through /api/admin-app-notifications",
        "one admin-gated GET load through /api/admin-app-notifications for the exact fake booking reference",
        "one exact cleanup delete scoped to admin_app_notification_outbox by event_key and booking_reference",
      ...
      verificationReference: fakeBookingReference,
      writtenScope: {
        adminAppNotificationOutbox: "one clearly marked fake in-app notification row only",
        bookings: "none",
...
main().catch((error) => {
  ...
  emitEvidence({
    error: "unexpected_admin_app_notification_production_save_load_runner_failure_sanitized",
    ok: false,
    persistenceDefaultAfter: "off",
    stage: "admin-app-notification-production-verification",
  });
