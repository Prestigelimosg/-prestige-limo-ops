...
const approvalValue = "stage-monthly-invoice-issue-review-william-approved";
...
const fakeCustomerAccount = "SAFE ISSUE REVIEW VERIFY 20260608 ACCOUNT";
const fakeBillingMonth = "2026-06";
const fakeDraftStatusSnapshot = "pending_admin_review";
const fakeInitialIssueReviewStatus = "issue_review_pending";
const fakeUpdatedIssueReviewStatus = "ready_for_future_issue";
const fakeReadinessStatus = "ready";
const fakeReadyCount = 1;
const fakeBlockedCount = 0;
const fakeTotalCount = 1;
const fakeSourceDraftSummary = {
  blocked_count: fakeBlockedCount,
  ready_count: fakeReadyCount,
  total_count: fakeTotalCount,
  verification_scope: "safe_issue_review_only",
};
const fakeSafeIssueContext = {
  issue_summary: "Safe issue review verification row.",
  next_action: "Load back, update status, then clean up exact row.",
  review_status: "Controlled verification.",
};
const fakeSafeIssueNote = "Controlled live issue review verification only.";
const liveAttemptMarkerPath = path.join(
  os.tmpdir(),
  "prestige-monthly-invoice-issue-review-live-write-attempted.marker",
);
...
async function loadAndValidateEnv() {
  ...
  failSafely("monthly_invoice_issue_review_production_env_preflight_failed", {
...
async function loadHarness() {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "prestige-monthly-issue-review-live-"));
...
function issueReviewPayload(issueReviewStatus = fakeInitialIssueReviewStatus) {
  return {
    billing_month: fakeBillingMonth,
    blocked_count: fakeBlockedCount,
    customer_account: fakeCustomerAccount,
    draft_id: fakeDraftId,
    draft_status_snapshot: fakeDraftStatusSnapshot,
    ...
    source_draft_summary: fakeSourceDraftSummary,
    total_count: fakeTotalCount,
  };
}
...
function parentDraftPayload() {
  return {
    ...
    blocked_count: fakeBlockedCount,
    customer_account: fakeCustomerAccount,
    customer_id: "safe-issue-review-verify-customer",
    draft_status: fakeDraftStatusSnapshot,
    ready_count: fakeReadyCount,
    readiness_status: fakeReadinessStatus,
    safe_draft_context: {
      draft_summary: "Safe parent row for issue review verification.",
      next_action: "Clean up exact parent after issue review cleanup.",
    },
    safe_draft_note: "Controlled issue review parent row only.",
    source_grouping_summary: fakeSourceDraftSummary,
    source_surface: "admin_api",
    actor_role: "admin",
    actor_label: "Monthly issue review verification",
    total_count: fakeTotalCount,
...
function issueReviewRecordMatchesFake(row, status = fakeInitialIssueReviewStatus) {
  return (
    ...
    row?.draft_status_snapshot === fakeDraftStatusSnapshot &&
    row?.issue_review_status === status &&
    ...
    row?.source_draft_summary?.verification_scope === fakeSourceDraftSummary.verification_scope &&
...
function parentDraftRecordMatchesFake(row) {
  return (
    ...
    row?.blocked_count === fakeBlockedCount &&
    row?.customer_account === fakeCustomerAccount &&
    row?.draft_status === fakeDraftStatusSnapshot &&
    row?.ready_count === fakeReadyCount &&
    row?.readiness_status === fakeReadinessStatus &&
    row?.safe_draft_note === "Controlled issue review parent row only." &&
    row?.source_surface === "admin_api" &&
...
async function writeLiveAttemptMarker() {
  try {
    await writeFile(
      ...
      `Monthly invoice issue review controlled live write attempted for ${fakeDraftId}\n`,
      {
    ...
    if (error?.code === "EEXIST") {
      failSafely("controlled_monthly_invoice_issue_review_live_write_already_attempted");
    }
...
async function loadExactIssueRows(client) {
  ...
  if (error) {
    failSafely("controlled_monthly_invoice_issue_review_cleanup_preselect_failed_safely", {
...
async function loadExactParentDraftRows(client) {
  ...
  if (error) {
    failSafely("controlled_monthly_invoice_issue_review_parent_preselect_failed_safely", {
      productionDbTouched: true,
      verificationDraftId: fakeDraftId,
    });
  }
...
async function createExactParentDraft(client) {
  ...
  const existingIssueRows = await loadExactIssueRows(client);

  if (existingParents.length !== 0 || existingIssueRows.length !== 0) {
    failSafely("controlled_monthly_invoice_issue_review_fake_rows_already_exist", {
      existingIssueRows: existingIssueRows.length,
  ...
  if (error || !parentDraftRecordMatchesFake(data) || safeRecordContainsUnsafeFields(data)) {
    failSafely("controlled_monthly_invoice_issue_review_parent_create_failed_safely", {
      productionDbTouched: true,
      verificationDraftId: fakeDraftId,
    });
  }
...
async function cleanupExactFakeRows() {
  ...
  const issueRows = await loadExactIssueRows(client);

  if (
    issueRows.length !== 1 ||
    !issueReviewRecordMatchesFake(issueRows[0], fakeUpdatedIssueReviewStatus) ||
    safeRecordContainsUnsafeFields(issueRows[0])
  ) {
    failSafely("controlled_monthly_invoice_issue_review_cleanup_exact_match_failed", {
      matchedIssueRows: issueRows.length,
      productionDbTouched: true,
      verificationDraftId: fakeDraftId,
    });
  }

  const { data: deletedIssueRows, error: issueDeleteError } = await client
    .from("monthly_invoice_issue_reviews")
    .delete()
    .eq("draft_id", fakeDraftId)
    .eq("customer_account", fakeCustomerAccount)
    .eq("billing_month", fakeBillingMonth)
    .select("draft_id, customer_account, billing_month, issue_review_status");

  if (issueDeleteError || !Array.isArray(deletedIssueRows) || deletedIssueRows.length !== 1) {
    failSafely("controlled_monthly_invoice_issue_review_cleanup_delete_failed_safely", {
      deletedIssueRows: Array.isArray(deletedIssueRows) ? deletedIssueRows.length : 0,
      productionDbTouched: true,
      verificationDraftId: fakeDraftId,
    });
  }

  const postIssueCleanupRows = await loadExactIssueRows(client);

  if (postIssueCleanupRows.length !== 0) {
    failSafely("controlled_monthly_invoice_issue_review_cleanup_verify_absent_failed", {
      remainingIssueRows: postIssueCleanupRows.length,
      productionDbTouched: true,
      verificationDraftId: fakeDraftId,
    });
  }
  ...
  if (
    ...
    failSafely("controlled_monthly_invoice_issue_review_parent_cleanup_exact_match_failed", {
  ...
  if (parentDeleteError || !Array.isArray(deletedParentRows) || deletedParentRows.length !== 1) {
    failSafely("controlled_monthly_invoice_issue_review_parent_cleanup_delete_failed_safely", {
  ...
  if (postParentCleanupRows.length !== 0) {
    failSafely("controlled_monthly_invoice_issue_review_parent_cleanup_verify_absent_failed", {
  ...
  return {
    deletedIssueRows: deletedIssueRows.length,
    deletedParentRows: deletedParentRows.length,
    postIssueCleanupRows: postIssueCleanupRows.length,
...
async function main() {
  ...
  try {
    const parsed = harness.issueReviewPersistence.parseAdminMonthlyInvoiceIssueReviewCreatePayload(
      issueReviewPayload(),
    );

    if (!parsed.ok) {
      failSafely("safe_monthly_invoice_issue_review_payload_rejected_before_live_write", {
        verificationDraftId: fakeDraftId,
      });
    }
    ...
    const blockedAnonymous = await readResponse(
      await harness.route.GET(
        getRequest(
          `http://localhost/api/admin-monthly-invoice-issue-reviews?draft_id=${fakeDraftId}`,
          {},
        ),
      ),
    );
    const blockedCustomerReferer = await readResponse(
      await harness.route.POST(
        requestWithJson("http://localhost/api/admin-monthly-invoice-issue-reviews", issueReviewPayload(), {
          ...adminHeaders(),
    ...
    const unsafePayloadResult = await readResponse(
      await harness.route.POST(
        requestWithJson(
          ...
          {
            ...issueReviewPayload(),
            safe_issue_note: "Send PDF payment link.",
          },
          adminHeaders(),
        ),
      ),
    ...
    if (
      ...
      failSafely("monthly_invoice_issue_review_route_safety_gate_failed_before_live_write", {
    ...
    const preExistingLoad = await readResponse(
      await harness.route.GET(
        getRequest(
          `http://localhost/api/admin-monthly-invoice-issue-reviews?draft_id=${fakeDraftId}&limit=10&page=1`,
          adminHeaders(),
        ),
      ),
    );
    ...
    if (
      ...
      !Array.isArray(preExistingLoad.body.issue_reviews) ||
      preExistingLoad.body.issue_reviews.length !== 0
    ) {
      failSafely("monthly_invoice_issue_review_fake_reference_already_exists_or_preload_failed", {
        matchedRows: Array.isArray(preExistingLoad.body?.issue_reviews)
          ? preExistingLoad.body.issue_reviews.length
          : null,
        productionDbTouched: true,
        verificationDraftId: fakeDraftId,
      });
    ...
    if (
      ...
      !issueReviewRecordMatchesFake(saveResult.body.issue_review) ||
      safeRecordContainsUnsafeFields(saveResult.body.issue_review)
    ) {
      failSafely("controlled_monthly_invoice_issue_review_save_failed_safely", {
        productionDbTouched: true,
    ...
    const loadResult = await readResponse(
      await harness.route.GET(
        getRequest(
          `http://localhost/api/admin-monthly-invoice-issue-reviews?draft_id=${fakeDraftId}&issue_review_status=${fakeInitialIssueReviewStatus}&limit=10&page=1`,
          adminHeaders(),
        ),
      ),
    );
    const loadedRecord = Array.isArray(loadResult.body?.issue_reviews)
      ? loadResult.body.issue_reviews.find((record) => issueReviewRecordMatchesFake(record))
      : null;
    ...
    if (
      ...
      failSafely("controlled_monthly_invoice_issue_review_load_failed_safely", {
        productionDbTouched: true,
    ...
    const updateResult = await readResponse(
      await harness.route.PATCH(
        requestWithJson(
          "http://localhost/api/admin-monthly-invoice-issue-reviews",
          issueReviewPayload(fakeUpdatedIssueReviewStatus),
          adminHeaders(),
          "PATCH",
        ),
      ),
    ...
    if (
      ...
      !issueReviewRecordMatchesFake(updateResult.body.issue_review, fakeUpdatedIssueReviewStatus) ||
      safeRecordContainsUnsafeFields(updateResult.body.issue_review)
    ) {
      failSafely("controlled_monthly_invoice_issue_review_update_failed_safely", {
        productionDbTouched: true,
    ...
    if (
      ...
      !Array.isArray(postCleanupRouteLoad.body.issue_reviews) ||
      postCleanupRouteLoad.body.issue_reviews.length !== 0
    ) {
      failSafely("controlled_monthly_invoice_issue_review_post_cleanup_route_load_failed", {
        matchedRows: Array.isArray(postCleanupRouteLoad.body?.issue_reviews)
          ? postCleanupRouteLoad.body.issue_reviews.length
          : null,
        productionDbTouched: true,
        verificationDraftId: fakeDraftId,
      });
    ...
    if (normalizedEnvValue(finalEnv.PRESTIGE_ADMIN_BOOKING_PERSISTENCE_ENABLED) !== "false") {
      failSafely("local_env_persistence_default_not_off_after_monthly_invoice_issue_review_verification", {
        productionDbTouched: true,
        verificationDraftId: fakeDraftId,
      });
    }
    ...
    emitEvidence({
      cleanupRollback: {
        ...
        cleanupMethod:
          "Supabase JS exact draft_id, customer_account, and billing_month delete on monthly_invoice_issue_reviews and monthly_invoice_drafts",
        cleanupScope: ["draft_id", "customer_account", "billing_month"],
        deletedIssueRows: cleanupResult.deletedIssueRows,
        deletedParentRows: cleanupResult.deletedParentRows,
        envFileChanged: false,
        persistenceDefaultAfter: "off",
        postCleanupDirectIssueRows: cleanupResult.postIssueCleanupRows,
      ...
      fakeMonthlyInvoiceIssueReview: {
        billing_month: fakeBillingMonth,
        customer_account: fakeCustomerAccount,
        draft_id: fakeDraftId,
        issue_review_status_after_update: fakeUpdatedIssueReviewStatus,
        issue_review_status_before_update: fakeInitialIssueReviewStatus,
        readiness_status: fakeReadinessStatus,
        safeFieldsOnly: true,
      },
      ...
      stage: "monthly-invoice-issue-review-production-verification",
      ...
      touchScope: [
        ...
        "one admin-gated POST save through /api/admin-monthly-invoice-issue-reviews",
        "one admin-gated GET load through /api/admin-monthly-invoice-issue-reviews for the exact fake draft id",
        "one admin-gated PATCH update through /api/admin-monthly-invoice-issue-reviews for the exact fake draft id",
        "one exact cleanup delete scoped to monthly_invoice_issue_reviews by draft_id, customer_account, and billing_month",
        "one exact cleanup delete scoped to monthly_invoice_drafts by id, customer_account, and billing_month",
        "one admin-gated GET load after cleanup to confirm no exact fake issue-review row remains",
      ],
      ...
      writtenScope: {
        ...
        monthlyInvoiceDrafts:
          "one exact fake parent row only, created for the required issue-review foreign key and then deleted",
        monthlyInvoiceIssueReviews: "one clearly marked fake issue-review row only",
        payments: "none",
        payouts: "none",
      },
    });
...
main().catch((error) => {
  ...
  emitEvidence({
    error: "unexpected_monthly_invoice_issue_review_production_save_load_runner_failure_sanitized",
    ok: false,
    persistenceDefaultAfter: "off",
    stage: "monthly-invoice-issue-review-production-verification",
  });
