...
function requireApproval() {
  if (envValue(approvalEnvName) !== expectedApproval) {
    throw new EvidenceFailure("driver_live_location_admin_runtime_evidence_not_approved", {
  ...
  if (!allowedPhases.has(phase)) {
    throw new EvidenceFailure("driver_live_location_admin_runtime_phase_not_approved", {
...
function stagingTargetUrl() {
  ...
  if (
    ...
    throw new EvidenceFailure("driver_live_location_admin_runtime_target_not_staging", {
...
function requireRuntimeWindowEnvNames() {
  ...
  if (missing.length > 0) {
    throw new EvidenceFailure("missing_required_driver_live_location_admin_runtime_env_names", {
  ...
  for (const name of [evidenceReferenceEnvName, bookingReferenceEnvName]) {
    ...
    if (!safeReferencePattern.test(value) || forbiddenSafeTextPattern.test(value)) {
      throw new EvidenceFailure("driver_live_location_admin_runtime_reference_invalid", {
        env_name: name,
      });
    }
  }
...
function tokenHash(rawToken) {
  ...
  if (cleanToken.length < 16) {
    throw new EvidenceFailure("driver_live_location_admin_runtime_token_invalid", {
...
async function expectPreOrPostClosed(target) {
  const driverToken = "driver-live-location-admin-runtime-closed-proof";
  ...
  const driverPost = await fetchJsonOrText(driverShareUrl, {
    body: JSON.stringify({
      accuracy_meters: 10,
      latitude: 1.2948,
      longitude: 103.8545,
...
async function readCurrentRuntimeSetting(client) {
  ...
  if (error) {
    throw new EvidenceFailure("driver_live_location_admin_runtime_setting_read_failed");
  }
...
async function restoreRuntimeSetting(client, previousSetting) {
  if (previousSetting) {
    ...
    if (error) {
      throw new EvidenceFailure("driver_live_location_admin_runtime_setting_restore_failed");
    }
  ...
  if (error) {
    throw new EvidenceFailure("driver_live_location_admin_runtime_setting_cleanup_failed");
  }
}
...
async function countEvidenceRows(client, tableName, evidenceReference) {
  ...
  if (error) {
    throw new EvidenceFailure("driver_live_location_admin_runtime_count_failed", {
...
async function insertEvidenceDriverLink(client, rawToken, bookingReference) {
  ...
  const { data, error } = await client
    .from(driverJobLinkTable)
    .insert({
      actor_label: "Driver live-location admin runtime evidence",
      actor_role: "admin",
  ...
  if (error || !data?.id) {
    throw new EvidenceFailure("driver_live_location_admin_runtime_driver_link_insert_failed");
  }

  return data.id;
}

async function openAdminRuntimeSetting(client, bookingReference) {
  ...
  if (error) {
    throw new EvidenceFailure("driver_live_location_admin_runtime_setting_open_failed");
  }
}

async function callDriverShare(target, rawToken) {
  const driverShareUrl = new URL(`/api/driver-job/${encodeURIComponent(rawToken)}/live-location`, target);
  const result = await fetchJsonOrText(driverShareUrl, {
    body: JSON.stringify({
      accuracy_meters: 10,
  ...
  if (result.status !== 200 || result.body?.ok !== true) {
    throw new EvidenceFailure("driver_live_location_admin_runtime_driver_share_failed", {
      status: result.status,
    });
  }

  if (result.body?.customerVisible !== false || result.body?.external_send !== false) {
    throw new EvidenceFailure("driver_live_location_admin_runtime_driver_share_unsafe_flags");
  }
...
async function callAdminMap(target) {
  const adminMapUrl = new URL("/api/admin-active-jobs-map-locations", target);
  ...
  if (result.status !== 200 || result.body?.ok !== true) {
    throw new EvidenceFailure("driver_live_location_admin_runtime_map_read_failed", {
      status: result.status,
    });
  }
  ...
  if (
    ...
    throw new EvidenceFailure("driver_live_location_admin_runtime_map_read_unsafe_payload");
  }
...
async function expectBlockedBoundaries(target, rawToken) {
  ...
  const driverDeleteUrl = new URL(`/api/driver-job/${encodeURIComponent(rawToken)}-wrong/live-location`, target);
  const wrongDriver = await fetchJsonOrText(driverDeleteUrl, {
    method: "DELETE",
  });

  if (wrongOrigin.status !== 403 || missingAdmin.status !== 403 || wrongDriver.status !== 401) {
    throw new EvidenceFailure("driver_live_location_admin_runtime_boundary_block_failed", {
...
async function runRuntimeWindow(target) {
  ...
  try {
    ...
    await openAdminRuntimeSetting(client, bookingReference);

    const driverShare = await callDriverShare(target, rawToken);
    const adminMap = await callAdminMap(target);
    const blocked = await expectBlockedBoundaries(target, rawToken);
    ...
    if (latestAfterCleanup !== 0 || auditAfterCleanup !== 0) {
      throw new EvidenceFailure("driver_live_location_admin_runtime_cleanup_failed", {
...
async function main() {
  ...
  if (phase === "pre-window" || phase === "post-rollback") {
    return {
      db_write: false,
      evidence_run: "driver_live_location_admin_runtime_closed_proof",
      phase,
      provider_send: false,
  ...
  return {
    db_write: "fake_admin_runtime_evidence_rows_only",
    evidence_run: "driver_live_location_admin_runtime_window",
    phase,
...
try {
  ...
  console.error(
    JSON.stringify(
      {
        error: "driver_live_location_admin_runtime_evidence_failed_safely",
        ok: false,
      },
      null,
      2,
