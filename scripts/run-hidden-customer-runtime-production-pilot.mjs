...
const approvalEnvName = "PRESTIGE_HIDDEN_CUSTOMER_PRODUCTION_PILOT_APPROVED";
const approvalValue = "hidden-customer-production-pilot-approved";
...
const liveAttemptMarkerPath = path.join(
  os.tmpdir(),
  "prestige-hidden-customer-runtime-production-pilot-attempted.marker",
);
const targetLabel = "hidden active customer";
...
async function loadAndValidateEnv() {
  ...
  failSafely("hidden_customer_production_pilot_env_preflight_failed", {
...
async function loadHarness() {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "prestige-hidden-customer-runtime-pilot-"));
...
function assertNoUnsafePayload(value, label) {
  ...
  if (forbiddenPayloadPattern.test(text)) {
    failSafely("hidden_customer_production_pilot_forbidden_payload", {
      label,
    });
  }
}
...
function assertSafePortalProjection(rows) {
  if (!Array.isArray(rows) || rows.length !== 1) {
    failSafely("hidden_customer_portal_expected_exactly_one_safe_booking", {
  ...
  if (unsafeKeys.length > 0) {
    failSafely("hidden_customer_portal_projection_unsafe_field", {
...
function assertSafeCustomerNotificationProjection(rows) {
  if (!Array.isArray(rows) || rows.length !== 1) {
    failSafely("hidden_customer_customer_in_app_expected_exactly_one_safe_row", {
      rowCount: Array.isArray(rows) ? rows.length : null,
    });
  }
  ...
  if (unsafeKeys.length > 0) {
    failSafely("hidden_customer_customer_in_app_projection_unsafe_field", {
      unsafeFieldCount: unsafeKeys.length,
    });
  }
...
async function expectStatus(responsePromise, expected, label) {
  ...
  if (response.status !== expected) {
    failSafely("hidden_customer_production_pilot_unexpected_status", {
      expected,
      label,
...
async function queryOrFail(query, label) {
  ...
  if (error) {
    failSafely("hidden_customer_production_pilot_db_query_failed_safely", {
      label,
    });
  }
...
async function findHiddenCustomerTarget(client) {
  ...
  if (rankedTargets.length === 0) {
    failSafely("hidden_customer_production_pilot_target_not_found", {
      targetLabel,
    });
  }
  ...
  if (!bookingReference) {
    failSafely("hidden_customer_production_pilot_booking_reference_not_safe");
  }
...
async function cleanupPilotRows(client, fixture) {
  ...
  if (notificationDelete.error) {
    failSafely("hidden_customer_notification_cleanup_failed_safely");
  }
  ...
  if (accountDelete.error) {
    failSafely("hidden_customer_access_mapping_cleanup_failed_safely");
  }
}
...
async function verifyZeroRows(client, fixture) {
  ...
  if (notificationCount.error) {
    failSafely("hidden_customer_notification_zero_row_check_failed_safely");
  }
  ...
  if (accountCount.error) {
    failSafely("hidden_customer_access_mapping_zero_row_check_failed_safely");
  }
...
async function createAccessMapping(client, fixture) {
  const insert = await client.from("customer_access_accounts").insert({
    ...
    safe_display_label: "Hidden customer controlled production pilot",
    source_surface: "system",
  });

  if (insert.error) {
    failSafely("hidden_customer_access_mapping_create_failed_safely");
  }
}
...
async function writeLiveAttemptMarker() {
  try {
    await writeFile(
      ...
      "Controlled hidden customer runtime production pilot attempted.\n",
      {
    ...
    if (error?.code === "EEXIST") {
      failSafely("hidden_customer_production_pilot_already_attempted");
    }
...
async function issueSessionCookie(harness, issueToken) {
  ...
  if (response.status !== 200 || body?.ok !== true) {
    failSafely("hidden_customer_customer_session_issue_failed_safely", {
      status: response.status,
    });
  }
  ...
  if (!cookiePair || cookiePair.length < 20) {
    failSafely("hidden_customer_customer_session_cookie_missing_safely");
  }
...
async function runRouteProofs(harness, fixture) {
  ...
  if (portalRead.status !== 200 || portalBody?.ok !== true) {
    failSafely("hidden_customer_portal_read_failed_safely", {
  ...
  if (
    ...
    failSafely("hidden_customer_out_of_scope_portal_read_failed_safely", {
  ...
  if (adminPost.status !== 200 || adminPostBody?.ok !== true) {
    failSafely("hidden_customer_admin_customer_in_app_write_failed_safely", {
  ...
  if (customerNotificationRead.status !== 200 || customerNotificationBody?.ok !== true) {
    failSafely("hidden_customer_customer_in_app_read_failed_safely", {
...
async function main() {
  if (process.env[approvalEnvName] !== approvalValue) {
    failSafely("hidden_customer_production_pilot_not_approved", {
  ...
  const target = await findHiddenCustomerTarget(client);
  const fixture = {
    authUserId: randomUUID(),
    bookingReference: target.bookingReference,
    customerAccountReference: target.customerAccountReference,
    eventKey: `hidden-customer-production-${randomUUID()}`,
    issueToken: `hidden-issue-${randomUUID()}-${randomUUID()}`,
    sessionToken: `hidden-session-${randomUUID()}-${randomUUID()}`,
  };
  ...
  try {
    ...
    if (!cleanup.zero_matching_rows) {
      failSafely("hidden_customer_production_pilot_cleanup_zero_row_failed");
    }

    emitEvidence({
      booking_scope: "one latest active hidden production customer booking",
      cleanup,
      db_write_scope: [
      ...
      evidence_reference: `HIDDEN-CUSTOMER-PRODUCTION-PILOT-${new Date()
        .toISOString()
        .replace(/[-:.TZ]/g, "")
        .slice(0, 14)}`,
      ...
      proof,
      rowIdsPrinted: false,
      stage: "hidden-customer-runtime-production-pilot",
...
main().catch((error) => {
  ...
  emitEvidence({
    ...
    error: "unexpected_hidden_customer_production_pilot_failure_sanitized",
    gates_closed_after: true,
    ok: false,
    stage: "hidden-customer-runtime-production-pilot",
  });
