...
assertIncludes(runner, 'const approvalValue = "stage-4a-393-william-approved";');
assertIncludes(runner, "STAGING-VERIFY-4A393-");
assertIncludes(runner, "prestige-stage4a393-controlled-live-write-attempted.marker");
assertIncludes(runner, 'stage: "4A-393"');
...
if (successEvidence) {
  for (const requiredText of [
    "Stage 4A-393",
    "b29a9e1 Confirm staging persistence readonly env key",
  ...
  assertMatches(successEvidence, /STAGING-VERIFY-4A393-\d{14}-[A-Z0-9]{2,}/);
  assertIncludes(docsIndex, "admin-persistence-staging-save-load-success-evidence.md");
  assertNotMatches(
    successEvidence,
    /SUPABASE_URL\s*=|SUPABASE_SERVICE_ROLE_KEY\s*=|SESSION_TOKEN\s*=|https:\/\/[^\s)`]+\.supabase\.co|eyJ[A-Za-z0-9_-]{20,}/,
  );
...
console.log("Admin persistence staging save-load evidence audit passed.");
