...
const routePath = "app/api/admin-production-deployment-hardening-readiness-preview-setup/route.ts";
...
function apiUrl(params = {}) {
  const url = new URL(
    "http://localhost/api/admin-production-deployment-hardening-readiness-preview-setup",
  );
...
async function loadHarness() {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "prestige-production-hardening-api-"));
...
for (const fragment of [
  ...
  "externalApiEnabled",
  "providerEnvEnabled",
  "paymentActivationEnabled",
  "authActivationEnabled",
  "liveSendingEnabled",
  "manualApprovalRequired",
]) {
  assert.ok(routeAndHelperSource.includes(fragment), `Missing production hardening readiness API fragment: ${fragment}`);
}
...
try {
  ...
  assert.equal(anonymousResponse.status, 403, "Production hardening readiness preview API must stay admin-gated.");
  assert.equal(anonymous.ok, false);
  assert.equal(anonymous.status, "blocked");
  assertProductionHardeningDisabled(anonymous, "Anonymous blocked response");
  assertPreview(anonymous.preview, "Anonymous blocked preview");
  assertReadiness(anonymous.readiness, "Anonymous blocked readiness");
  assertProductionHardeningDisabled(anonymous.setup, "Anonymous blocked setup");
  ...
  assertProductionHardeningDisabled(ready, "Ready response");
  assertPreview(ready.preview, "Ready preview");
  assertReadiness(ready.readiness, "Ready readiness");
  assertProductionHardeningDisabled(ready.setup, "Ready setup");
  ...
  assert.equal(ready.setup.deployment_label, "prod-readiness-2026-06");
  assert.equal(ready.setup.release_candidate, "release-candidate-setup-1");
  ...
  assertProductionHardeningDisabled(unsafe, "Unsafe response");
...
console.log("admin production deployment hardening readiness preview setup API contract passed");
