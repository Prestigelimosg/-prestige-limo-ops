...
const routePath = "app/api/admin-sms-customer-driver-details-preview-readiness-setup/route.ts";
...
function apiUrl(params = {}) {
  const url = new URL("http://localhost/api/admin-sms-customer-driver-details-preview-readiness-setup");
...
async function loadHarness() {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "prestige-sms-customer-driver-details-api-"));
...
for (const fragment of [
  ...
  assert.ok(routeAndHelperSource.includes(fragment), `Missing SMS customer driver-details API fragment: ${fragment}`);
}
...
try {
  ...
  assert.equal(anonymousResponse.status, 403, "SMS customer driver-details API must stay admin-gated.");
  ...
  assert.equal(anonymous.readiness.smsMessageReady, false);

  const readyResponse = await harness.route.GET(
    new Request(
      apiUrl({
        booking_reference: "PLO-SMS-API-001",
        driver_name: "Tan Driver",
        driver_phone: "+65 8888 0000",
        pickup_time: "12 Jun 2026, 10:00",
        secure_details_link: "https://prestige.example/customer-driver-details/PLO-SMS-API-001",
  ...
  assert.deepEqual(ready.preview.message, {
    message_key: "customer_assigned_driver_details_sms",
    message_text:
      "Prestige: Booking PLO-SMS-API-001. Pickup 12 Jun 2026, 10:00. Driver Tan Driver +65 8888 0000. Vehicle Mercedes V-Class SLA1234X. Details https://prestige.example/customer-driver-details/PLO-SMS-API-001.",
  ...
  const camelResponse = await harness.route.GET(
    new Request(
      apiUrl({
        bookingReference: "PLO-SMS-API-002",
        detailsLink: "https://prestige.example/customer-driver-details/PLO-SMS-API-002",
  ...
  assert.equal(camel.preview.payload.booking_reference, "PLO-SMS-API-002");
  ...
  assert.equal(
    ...
    "https://prestige.example/customer-driver-details/PLO-SMS-API-002",
  );
  ...
  const incompleteResponse = await harness.route.GET(
    new Request(
      apiUrl({
        booking_reference: "PLO-SMS-API-003",
        driver_name: "Tan Driver",
      }),
      { headers: adminHeaders() },
    ),
  ...
  assert.equal(
    incomplete.preview.message.message_text,
    "Prestige: Booking PLO-SMS-API-003. Driver Tan Driver.",
  );
  ...
  assert.equal(
    unsafe.preview.message.message_text,
    "Prestige: Pickup 10:00. Vehicle Mercedes SLA1234X.",
  );
  assert.equal(
    unsafeOutputPattern.test(JSON.stringify(unsafe)),
    false,
    "Unsafe SMS customer driver-details API output must not leak pricing, payout, billing, payment, internal, provider, or token details.",
  );
} finally {
  restoreEnv();
...
console.log("admin SMS customer driver details preview readiness setup API contract passed");
