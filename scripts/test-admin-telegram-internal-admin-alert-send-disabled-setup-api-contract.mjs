...
const routePath = "app/api/admin-telegram-internal-admin-alert-send-disabled-setup/route.ts";
...
function apiUrl(params = {}) {
  const url = new URL("http://localhost/api/admin-telegram-internal-admin-alert-send-disabled-setup");
...
function assertBlockedSend(value, label) {
  ...
  assert.equal(value.no_op, true, `${label} must stay a no-op.`);
  assert.equal(value.result_label, "blocked/no-op", `${label} must expose blocked/no-op result.`);
}
...
async function loadHarness() {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "prestige-admin-telegram-alert-send-disabled-api-"));
...
for (const fragment of [
  ...
  assert.ok(routeAndHelperSource.includes(fragment), `Missing disabled Telegram alert API fragment: ${fragment}`);
}
...
try {
  ...
  assert.equal(anonymousResponse.status, 403, "Disabled Telegram admin alert send API must stay admin-gated.");
  ...
  assertBlockedSend(defaultResult.send, "Default send result");
  assert.equal(defaultResult.status, "blocked");
  ...
  assert.equal(defaultResult.send.event_type, null);
  assert.equal(defaultResult.send.disabled_adapter.status, "disabled");
  ...
  const driverAckResponse = await harness.route.GET(
    new Request(
      apiUrl({
        ...
        booking_reference: "PLO-TG-SEND-001",
  ...
  assertBlockedSend(driverAck.send, "Driver ack send result");
  assert.equal(driverAck.status, "blocked");
  assert.equal(driverAck.event_type, "driver_ack_customer_message_ready");
  assert.equal(driverAck.preview.safe_message, "Driver acknowledgement customer message is ready for PLO-TG-SEND-001.");
  assert.equal(driverAck.send.event_key, "driver_ack_customer_message_ready-PLO-TG-SEND-001");
  assert.equal(driverAck.send.notification_type, "driver_ack_customer_message_ready");
  assert.equal(
    driverAck.send.preview.safe_message,
    "Driver acknowledgement customer message is ready for PLO-TG-SEND-001.",
  );
  ...
  const customerEmailResponse = await harness.route.GET(
    new Request(
      apiUrl({
        ...
        bookingReference: "PLO-TG-SEND-002",
  ...
  assertBlockedSend(customerEmail.send, "Customer driver-details email send result");
  ...
  assert.equal(customerEmail.send.event_key, "customer_driver_details_email_ready-PLO-TG-SEND-002");
  assert.equal(customerEmail.readiness.alertReadyForFutureSetup, true);

  const urgentResponse = await harness.route.GET(
    new Request(
      apiUrl({
        booking_reference: "PLO-TG-SEND-003",
  ...
  assertBlockedSend(urgent.send, "Urgent review send result");
  assert.equal(urgent.event_type, "urgent_review_required");
  assert.equal(urgent.preview.safe_title, "Urgent admin review required");
  assert.equal(urgent.preview.safe_message, "Urgent admin review is required for PLO-TG-SEND-003.");
  ...
  assert.equal(unsafe.send.event_key, null);
  assert.equal(unsafe.send.preview.safe_message, null);
  assert.equal(unsafe.send.preview.safe_title, null);
  ...
  assert.equal(
    ...
    false,
    "Unsafe disabled Telegram alert send API output must not leak live-send, token, payment, or payout text.",
  );
} finally {
  restoreEnv();
...
console.log("admin Telegram internal admin alert disabled send setup API contract passed");
