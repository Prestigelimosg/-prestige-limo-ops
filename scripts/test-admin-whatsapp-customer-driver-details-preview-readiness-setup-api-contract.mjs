...
const routePath = "app/api/admin-whatsapp-customer-driver-details-preview-readiness-setup/route.ts";
...
function apiUrl(params = {}) {
  const url = new URL("http://localhost/api/admin-whatsapp-customer-driver-details-preview-readiness-setup");
...
async function loadHarness() {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "prestige-whatsapp-customer-driver-details-api-"));
...
for (const fragment of [
  ...
  assert.ok(routeAndHelperSource.includes(fragment), `Missing WhatsApp customer driver-details API fragment: ${fragment}`);
}
...
try {
  ...
  assert.equal(anonymousResponse.status, 403, "WhatsApp customer driver-details API must stay admin-gated.");
  ...
  assert.equal(anonymous.readiness.customerMessageReady, false);

  const readyResponse = await harness.route.GET(
    new Request(
      apiUrl({
        booking_reference: "PLO-WA-API-001",
  ...
  assert.deepEqual(ready.preview.message.body_lines, [
    "Booking: PLO-WA-API-001",
  ...
  assert.equal(
    ready.preview.message.message_text,
    [
  ...
  assert.equal(ready.preview.disabled_message.status, "disabled");
  assert.equal(ready.preview.disabled_message.external_send, false);

  const camelResponse = await harness.route.GET(
    new Request(
      apiUrl({
        bookingReference: "PLO-WA-API-002",
  ...
  assert.equal(camel.preview.payload.booking_reference, "PLO-WA-API-002");
  ...
  assert.equal(incomplete.preview.disabled_message.status, "disabled");
  assert.equal(incomplete.preview.disabled_message.external_send, false);
  ...
  assert.equal(
    ...
    false,
    "Unsafe WhatsApp customer driver-details API output must not leak pricing, payout, billing, payment, internal, provider, or token details.",
  );
} finally {
  restoreEnv();
...
console.log("admin WhatsApp customer driver details preview readiness setup API contract passed");
