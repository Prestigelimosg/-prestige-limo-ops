...
assert.equal(
  ...
  true,
  "The OTP provider and credentials must remain inside one server-only Twilio Verify helper.",
);

assert.equal(
  phoneRouteSource.includes(
  ...
  true,
  "The one OTP route must enforce the existing /book same-origin purpose boundary and no-store responses.",
);
...
assert.equal(
  ...
  true,
  "The existing booking route must preserve invitation priority and require a bound phone proof only for public no-invite requests.",
);
...
assert.equal(
  ...
  true,
  "The proof must remain a header and never enter the persisted booking payload.",
);
...
assert.equal(
  ...
  true,
  "The server-only OTP table must be RLS-protected, least-privilege, atomically rate-limited, and security-invoker only.",
);
...
assert.equal(
  ...
  false,
  "The anti-abuse table must not store raw phone numbers, IP addresses, OTP codes, or provider credentials.",
);

assert.equal(
  ledgerSource.includes("Public First-Booking Phone Verification") &&
    ledgerSource.includes("private invitation remains no-OTP") &&
    ledgerSource.includes("Customer Portal remains no-OTP"),
  true,
  "The ledger must record the exact additive OTP boundary and preserved no-OTP lanes.",
);
...
try {
  ...
  const providerFetch = async (url, init) => {
    state.fetchCalls.push({ body: String(init.body), headers: init.headers, url });
  ...
  assert.match(
    state.rpcCalls[0].args.p_phone_hash,
    /^[a-f0-9]{64}$/,
  );
  ...
  assert.match(verifiedProof.data.booking_reference, /^CBOTP-[A-F0-9]{24}$/);
  assert.equal(
  ...
  const adapterCalls = [];
  const adapterStart = await adapter.startCustomerBookingPhoneOtpVerification(
    "+65 9000 1111",
    {
      fetcher: async (url, init) => {
        adapterCalls.push({ body: JSON.parse(init.body), headers: init.headers, url });
        return new Response(
          JSON.stringify({
