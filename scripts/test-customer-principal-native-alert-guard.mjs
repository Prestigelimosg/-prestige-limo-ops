...
assert.match(activation, /action: "complete_activation"/);
assert.match(activation, /Create 6-digit PIN/);
assert.match(activation, /window\.location\.assign\("\/my-bookings"\)/);
assert.doesNotMatch(activation, /start_activation|Verify invited email|One-time email code|localStorage\.setItem\([^\n]*(?:invitation|pin|confirmPin)/i);
