...
try {
 ...
 if(['/assigned','/unassigned','/invalid','/wired-assigned','/wired-unassigned'].includes(location.pathname))window.__PRESTIGE_DRIVER_ACCOUNT_ACTIVATION__={setupId:'22222222-2222-4222-8222-222222222222',email:'driver@example.test',password:'482951',activated:false,jobUrl:'https://app.prestigelimo.sg/driver-job/'+'a'.repeat(64)};
 function Job(){const [ack,setAck]=useState(false),[ready,setReady]=useState(false);return <><DriverAccountActivation token={'a'.repeat(64)} acknowledged={ack} onReady={account=>{window.test.ready.push(account);setReady(true);}}/><button disabled={!ready||ack} onClick={()=>setAck(true)}>Save & Acknowledge Job</button></>;}
