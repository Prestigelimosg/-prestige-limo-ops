...
try {
 ...
 assert.equal(admin.data.offer.responses[0].status,'available');assert.doesNotMatch(JSON.stringify(admin),/SECRET|DO_NOT_EXPOSE|safe_bid_context|safe_offer_context/);
 ...
 const driverRoute=load('app/api/driver-job-bids/route.ts',{
  'next/server':{after:cb=>after.push(cb)},
  '../../../lib/admin-device-push-notification':{sendAdminDevicePushAlert:async()=>{throw Error('Unexpected winner alert');}},
  '../../../lib/driver-account-device-lock':{verifyDriverAccountSession:async()=>verified},
  '../../../lib/driver-device-push-notification':modules['./driver-device-push-notification'],
  '../../../lib/driver-pool-fast-accept':helper,
 ...
 console.log('Driver Pool Admin selection guard passed: strict inputs, recipient privacy, response readback, verified award/widen routes, rejected/stale business results and exact mocked notification recipients.');
