...
try {
  ...
  for (const id of [1,2,3,4,5]) assert.equal((await respond(o,id)).reason,'awaiting_admin');
  ...
  assert.equal((await list(1)).jobs[0].response_status,'awaiting_admin');
  assert.equal((await respond(o,1)).reason,'awaiting_admin');
  await assert.rejects(widen(o));
  const winKey=randomUUID(); assert.equal((await award(o,3,winKey)).reason,'accepted');
  assert.equal((await award(o,3,winKey)).reason,'already_accepted');
  assert.equal((await award(o,2)).ok,false); assert.equal(await assigned(),3);
  ...
  assert.equal((await respond(wide,6)).reason,'awaiting_admin'); assert.equal(await assigned(),null);
  assert.equal((await award(wide,6)).reason,'accepted');
  for (const mutation of [
    ...
    await reset(); o=await publish(); await respond(o,1); await db.exec(mutation);
    assert.equal((await award(o,1)).ok,false,mutation);
  ...
  await respond(o,1); await decline(o,1); assert.equal((await award(o,1)).ok,false);
  ...
  await reset(); o=await publish([1,2]); await respond(o,1); await award(o,1);
  ...
  if (connect) {
    ...
    const other=await publish([1],randomUUID(),'OTHER'); await respond(o,1); await respond(other,1);
    const clients=await Promise.all([connect(),connect()]);
    try {
      const results=await Promise.all([o,other].map((offer,i)=>clients[i].query("select accept_driver_pool_offer($1,1,$2,$3,'admin','Synthetic Admin') result",[offer.offer.offer_key,offer.offer.updated_at,randomUUID()])));
      assert.equal(results.filter(r=>r.rows[0].result.reason==='accepted').length,1);
    ...
    try {
      ...
      assert.ok(state.audience==='wider' ? response.response===null : response.response==='available');
    ...
    await reset(); o=await publish([1]); await respond(o,1);
    ...
    try {
      ...
      const pending=contender.query("select accept_driver_pool_offer($1,1,$2,$3,'admin','Synthetic Admin') result",[o.offer.offer_key,o.offer.updated_at,randomUUID()]);
  ...
  console.log('PASS selected recipients, five responses, one Admin award, fixed amount, decline rejection, same-offer widening, replay/stale checks, account/vehicle/schedule/booking guards, privacy and no link/status writes.');
