...
for (const fragment of [
  ...
  "loadAdminDriverPoolOffer(database.client, combo?.primary_booking_reference || reference)",
...
for (const fragment of [
  ...
  "exactKeys(record, [\"booking_reference\", \"expected_updated_at\", \"offer_payout_sgd\", \"idempotency_key\", \"vehicle_requirement\", \"selected_driver_ids\", \"audience\", \"combo_revision\"])",
  "exactKeys(record, [\"offer_key\", \"expected_updated_at\"])",
  "exactKeys(record, [\"offer_key\", \"expected_updated_at\", \"idempotency_key\"])",
  ".rpc(combo ? \"publish_driver_job_combo\" : \"publish_driver_pool_offer\"",
  ".rpc(comboOffer ? \"cancel_driver_job_combo_offer\" : \"cancel_driver_pool_offer\"",
  ".rpc(action === \"accept\" ? comboAcceptance ? \"accept_driver_job_combo\" : \"accept_driver_pool_offer\" : \"decline_driver_pool_offer\"",
