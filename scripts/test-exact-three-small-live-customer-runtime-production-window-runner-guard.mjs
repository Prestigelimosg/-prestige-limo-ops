...
const thisGuardPath =
  "scripts/test-exact-three-small-live-customer-runtime-production-window-runner-guard.mjs";
...
const exactThreeSection = sectionBetween(
  ledger,
...
for (const fragment of [
  ...
  "exact-3-small-live-customer-runtime-window-approved",
  ...
  "exactThreeAllowlistSize = 3",
  "exactThreeProfileRequested",
  "selectExactThreeLiveWindowTargets",
  ...
  assertIncludes(runner, fragment, `runner exact-3 fragment ${fragment}`);
}

for (const fragment of [
  "exactThreeCustomerRuntimeSessionMapEntryCount = 3",
  ...
  assertIncludes(helper, fragment, `session-map exact-3 fragment ${fragment}`);
}
...
for (const fragment of [
  ...
  assertIncludes(customerSavedBookingsRead, fragment, `saved-bookings exact-3 boundary ${fragment}`);
}
...
for (const fragment of [
  ...
  assertIncludes(
    customerAppNotificationPersistence,
    fragment,
    `customer in-app exact-3 boundary ${fragment}`,
  );
}

for (const fragment of [
  "This is a disabled-by-default exact-3 extension",
  "The exact-2 live-window evidence history remains preserved",
  "The exact-3 profile requires `PRESTIGE_EXACT3_SMALL_LIVE_CUSTOMER_RUNTIME_WINDOW_APPROVED=exact-3-small-live-customer-runtime-window-approved`",
  "The exact-3 execution phase is disabled by default",
  ...
  assertIncludes(exactThreeSection, fragment, `ledger exact-3 fragment ${fragment}`);
}
...
for (const forbiddenPattern of [
  ...
  assertExcludes(runner, forbiddenPattern, "exact-3 runner forbidden surface");
  assertExcludes(exactThreeSection, forbiddenPattern, "exact-3 ledger forbidden surface");
}

assertIncludes(preactivationSuite, thisGuardPath, "preactivation suite exact-3 registration");

console.log("Exact-3 small live customer production runtime window runner guard passed.");
