-- Repair only the existing-Booker branch of the established atomic Admin
-- Customer + Company + Booker profile transaction. The function returns a
-- `company_id` output column, so every Booker predicate must use the explicit
-- target alias instead of an ambiguous unqualified column reference.
...
begin
  ...
  if p_booker_id is null then
    ...
    update public.bookers as target_booker set
      ...
      updated_at = now()
    where target_booker.id = p_booker_id
      and target_booker.company_id = v_company.id
      and (target_booker.customer_id is null or target_booker.customer_id = p_customer_id)
    returning target_booker.* into v_booker;
...
comment on function public.apply_admin_customer_company_booker_profile(
  ...
  'Server-only exact Admin Customer + Company + Booker profile transaction. Existing-Booker predicates are explicitly qualified; Traveller and all operational consumers remain untouched.';
