-- Local review: one NEW server-only transaction for the existing Create Link path.
-- Does not run a backfill, send notifications, change ACK or write Calendar/GPS.
create function public.apply_admin_driver_job_link(
