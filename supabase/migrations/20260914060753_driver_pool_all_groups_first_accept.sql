-- First-valid acceptance for both selected and wider Driver Pool groups.
-- No stored offers/bids are rewritten or auto-awarded. Historical pending offers
-- use the same effective mode on their next explicit read/acceptance.
begin;
do $baseline$ begin
  if not exists(select 1 from pg_proc where oid=to_regprocedure('public.accept_driver_pool_offer(text,bigint,timestamptz,text,text,text)')
    and not prosecdef and md5(regexp_replace(prosrc,'[[:space:]]','','g'))='4ce52574f448c36f5fee7478f4e72c10')
  or not exists(select 1 from pg_proc where oid=to_regprocedure('public.publish_driver_pool_offer(text,timestamptz,numeric,text,text,text,text,bigint[],text)')
    and not prosecdef and md5(regexp_replace(prosrc,'[[:space:]]','','g'))='281a384de303eac9694ca3e7b172953b')
  or not exists(select 1 from pg_proc where oid=to_regprocedure('public.list_driver_pool_available_jobs(bigint,integer,integer)')
    and not prosecdef and md5(regexp_replace(prosrc,'[[:space:]]','','g'))='13303b3abaf4a43ce6fa882af48f7deb') then
    raise exception 'Driver Pool baseline changed. Inspect before restoring first acceptance.';
  end if;
end $baseline$;
...
begin
  ...
  -- Stale Admin winner controls cannot award a driver in first-accept mode.
  if v_admin then
  ...
  -- Both selected and wider groups use the existing atomic first-winner path.
...
begin
  ...
  insert into public.driver_job_bid_offers (
    ...
    jsonb_build_object('kind','fixed_driver_offer','price_scope','driver_offer_only','vehicle_requirement_version',1,'selection_mode','first_accept','audience','selected',
