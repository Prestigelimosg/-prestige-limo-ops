-- Remove only the selected audience business cap; retain exact eligibility and first acceptance.
begin;
do $baseline$ begin
  if not exists(select 1 from pg_proc where oid=to_regprocedure('public.publish_driver_pool_offer(text,timestamptz,numeric,text,text,text,text,bigint[],text)')
    and not prosecdef and md5(regexp_replace(prosrc,'[[:space:]]','','g'))='81001eaa1de8b42247da34875a34d80c') then
    raise exception 'Driver Pool publisher baseline changed. Inspect before removing the selected driver cap.';
  end if;
end $baseline$;
...
begin
  ...
  if v_reference !~ '^[A-Za-z0-9][A-Za-z0-9._:-]{0,119}$'
     ...
     or (not v_widen and not v_direct_wider and (p_selected_driver_ids is null or cardinality(p_selected_driver_ids) < 1
