...
create function public.register_driver_native_push_installation(
...
begin
  ...
  if not exists (select 1 from public.driver_job_links where id=p_link_id and driver_id=p_driver_id
  ...
  on conflict(endpoint) do update set last_driver_job_link_id=excluded.last_driver_job_link_id,
